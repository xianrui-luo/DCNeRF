...
def train():
    ...
    render_kwargs_train, render_kwargs_test, start, grad_vars, optimizer,bokeh_param = create_nerf_final(args,N_image)
    ...
    #     # For random ray batching
        ...
        rays_rgb_main_patch= np.concatenate([rays, main_images[:,None]], 1) 
        rays_depth_patch= np.concatenate([rays, depth_gt[:,None]], 1) 
        
        rays_rgb_patch = np.transpose(rays_rgb_patch, [0,2,3,1,4]) 
        rays_rgb_main_patch = np.transpose(rays_rgb_main_patch, [0,2,3,1,4]) 
        rays_depth_patch = np.transpose(rays_depth_patch, [0,2,3,1,4])
        rays_rgb = np.stack([rays_rgb_patch[i] for i in i_train], 0) 
        rays_rgb_main = np.stack([rays_rgb_main_patch[i] for i in i_train], 0)
        rays_depth = np.stack([rays_depth_patch[i] for i in i_train], 0) 
        

        rays_rgb = np.reshape(rays_rgb, [-1,4,3]) 

        rays_rgb_main = np.reshape(rays_rgb_main, [-1,3,3]) 
        rays_depth = np.reshape(rays_depth, [-1,3,3]) 
    ...
    for i in trange(start, N_iters + args.bokeh_iters):
       ...
       # Random from one image
        ...
        if i<N_iters:
                    ...
                    for h in range(H_patch+2):
                        for w in range(W_patch+2):
                            ...
                            #     continue
                                ...
                                patch_main_rays.append(rays_rgb_main_patch[index, h0:h0+N_pixel, w0:w0+N_pixel, ...])
                                patch_depth_rays.append(rays_depth_patch[index, h0:h0+N_pixel, w0:w0+N_pixel, ...])
            ...
            min_defocus=torch.min(target_defocus)
            max_defocus=torch.max(target_defocus)
            target_defocus=(target_defocus-min_defocus)/(max_defocus-min_defocus)
        ...
        if i<N_iters:
            ...
            binary_mask=torch.where(target_focus<bokeh_range,zeros,target_focus)
            binary_mask=torch.where(target_focus>0.9,ones,binary_mask)

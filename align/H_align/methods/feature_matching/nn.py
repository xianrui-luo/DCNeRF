...
from H_align.utils.match_helper import get_matching_dist_type, remove_duplicate_matches
...
#    print('desc1: {} ----- desc2: {}'.format(desc1.shape, desc2.shape))
    if do_flann:
        if dist == cv2.NORM_L2:
    ...
    for m_idx, cur_match in enumerate(matches):
        valid_matches_0.append([cur_match[0].queryIdx, cur_match[0].trainIdx])
        valid_matches_1.append([cur_match[1].queryIdx, cur_match[1].trainIdx])
        valid_matches_2.append([cur_match[2].queryIdx, cur_match[2].trainIdx])
        valid_matches_3.append([cur_match[3].queryIdx, cur_match[3].trainIdx])
    matches_0 = np.expand_dims(np.asarray(valid_matches_0).T, axis=0)
    matches_1 = np.expand_dims(np.asarray(valid_matches_1).T, axis=0)
    matches_2 = np.expand_dims(np.asarray(valid_matches_2).T, axis=0)
    matches_3 = np.expand_dims(np.asarray(valid_matches_3).T, axis=0)
    matches = np.concatenate([matches_0, matches_1, matches_2, matches_3], axis=0)
